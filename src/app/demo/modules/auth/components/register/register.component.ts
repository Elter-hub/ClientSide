...
export class RegisterComponent implements OnInit {
  ...
  ngOnInit() {
    this.registrationForm = this.formBuilder.group({
      ...
      userEmail: ['ihor04@gmail.com', [Validators.required, /*Validators.pattern('^[a-z0-9._%+-]+@[a-z0-9.-]+\.[a-z]{2,4}$')*/]],
      matcher: this.formBuilder.group({
        userPassword: ['Superuser123', [Validators.required, /*Validators.pattern('(?=.*[a-z])(?=.*[A-Z])(?=.*[0-9]).{8,20}')*/]],
